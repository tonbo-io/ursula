...
powered by `ursulactl`. The default chart also separates the client/admin port
from the Raft peer port and adds soft multi-pod spread hints.
...
helm install ursula charts/ursula \
  --set image.repository=ursula \
  --set image.tag=dev \
  --set image.pullPolicy=Never
```

The default install starts three voters and 64 Raft groups. For a registry
image, set `image.repository`, `image.tag`, and optionally `image.pullSecrets`.
...
`replicaCount` controls the initial voter set for a fresh cluster. The chart
supports `1`, `3`, and `5`; production clusters should use `3` or `5`.

Changing `replicaCount` on an initialized cluster is not safe Raft voter
...
Kubernetes probes are disabled in the current chart (`FIXME` in
`statefulset.yaml`). Use `helm test` or `ursulactl wait-ready` for cluster-level
readiness.
