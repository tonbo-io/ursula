...
curl -X POST "http://127.0.0.1:4437/__ursula/raft/42/membership?voters=1,2,3"

# Put this node into maintenance drain / lift it again
curl -X POST   http://127.0.0.1:4437/__ursula/leadership-shed/maintenance
curl -X DELETE http://127.0.0.1:4437/__ursula/leadership-shed/maintenance

# Permit the leader to accept one log revert from a wiped, rejoining node
curl -X POST http://127.0.0.1:4437/__ursula/raft/42/nodes/4/allow-next-revert
```
