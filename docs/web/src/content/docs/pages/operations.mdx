...
# {"bucket":"tenant-a","removed_streams":12,"groups_with_streams":[0,3,5],"cold_gc_entries_reclaimed":12,"cold_gc_pending_entries":0,"cold_gc_complete":true,"cold_gc_error":null}
```
...
- **Cold-GC interaction.** `cold_gc_entries_reclaimed` is diagnostic only. Ursula issues a second idempotent replicated purge to read every group's durable queue after the immediate pass; only `cold_gc_complete=true` with `cold_gc_pending_entries=0` proves cold reclamation. If S3 is unavailable, the response remains incomplete and the queued entries survive for the background worker and a later proof request.
