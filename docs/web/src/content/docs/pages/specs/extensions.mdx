...
- `append-batch`, `attrs`, `bootstrap`, `retention`, and `snapshot` are reserved local stream IDs under an affinity path because those names identify stream subresources on the two-segment form.
