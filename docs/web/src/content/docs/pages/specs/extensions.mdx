...
Implementations MAY reclaim ordinal index entries only after the corresponding records fall before `F` and are no longer required by a visible snapshot, bootstrap response, or fork.
...
8. Flush, compaction, snapshot, restart, fork, and retention never change a surviving record's ordinal.
