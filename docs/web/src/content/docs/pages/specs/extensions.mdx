...
Servers implementing this extension MUST advertise `json-record-coordinates-v1` in a comma-separated `Stream-Extensions` response header on `HEAD` and stream-operation responses. Record-aware clients MUST verify this capability before relying on record query parameters or response headers and MUST reject a record-aware response that omits it.
...
- `C(r)` is the server-assigned commit timestamp of record `r`.
...
### 6.2. Commit Timestamps

The server MUST assign an RFC 3339 commit timestamp to every committed record. Commit timestamps MUST be nondecreasing within a stream. If the wall clock moves backwards, the server MUST clamp the new timestamp to at least the previous committed timestamp.

All records created by one atomic append MAY share one commit timestamp. Equal timestamps are ordered by record ordinal. Commit timestamps are immutable after assignment and MUST survive recovery and compaction.

Commit time is not application event time. Applications SHOULD continue to include domain event time in their JSON values when that distinction matters.
...
- `Stream-Commit-Timestamp`: Commit timestamp assigned to the operation.

For an operation that creates no records, `Stream-Record-Start` and `Stream-Record-Next` MUST both equal the current record tail and `Stream-Commit-Timestamp` MUST be omitted.
...
When an idempotent-producer retry is deduplicated, the server MUST return the original `Stream-Record-Start`, `Stream-Record-Next`, and `Stream-Commit-Timestamp` values.
...
Stream-Commit-Timestamp: 2026-07-18T10:30:00.123Z
...
- `Stream-Record-First-Timestamp`: Commit timestamp of record `F`, when the retained range is non-empty.
- `Stream-Record-Last-Timestamp`: Commit timestamp of record `N - 1`, when the retained range is non-empty.

The timestamp headers MUST be absent when `F = N`.
...
- `timestamp=<rfc3339>`: Start at the first retained record whose commit timestamp is greater than or equal to the supplied timestamp.
...
- A timestamp later than the last committed timestamp resolves to `N`.
- When `F > 0`, a timestamp earlier than `C(F)` MUST return `410 Gone` so trimmed history is not silently hidden.
...
- `timestamp`: RFC 3339 commit timestamp.
- `value`: The original normalized JSON value.

Example:

```json
{"record":42,"timestamp":"2026-07-18T10:30:00.123Z","value":{"type":"navigation_started"}}
{"record":43,"timestamp":"2026-07-18T10:30:00.123Z","value":{"type":"network_response","status":500}}
```
...
### 6.10. Retention, Snapshots, Bootstrap, and Forks

Retention MUST advance `F` only at a record boundary. A trimmed ordinal MUST return `410 Gone`; surviving records keep their original ordinals and commit timestamps.
...
When a new stream is forked from a JSON stream implementing this extension, the copied messages form the child stream's initial record sequence starting at ordinal `0`. The child does not inherit the parent's ordinal namespace. A fork offset MUST align with a source record boundary.

Implementations MAY reclaim ordinal and timestamp index entries only after the corresponding records fall before `F` and are no longer required by a visible snapshot, bootstrap response, or fork.
...
4. A deduplicated append returns its original ordinal range and commit timestamp.
...
8. Flush, compaction, snapshot, restart, fork, and retention never change a surviving record's ordinal or commit timestamp.
9. `N - F` equals the number of retained logical messages.
10. Timestamp navigation with ordinal tie-breaking is deterministic.
11. Base-protocol clients observe no semantic change.

Conformance tests SHOULD cover JSON array flattening, concurrent append ordering, idempotent retry, record-tail preconditions, partial offset reads, record-aligned reads, timestamp ties, retention, snapshot/bootstrap boundaries, cold flush, restart, and live-read reconnection.
