...
- `consistency` (optional)
 - `local` (default) permits immutable catch-up bytes from a follower's applied
   state. `leader` routes the read to the current Raft leader and is intended
   for recovery that must observe an acknowledged write before proceeding.
   `leader` cannot be combined with a live read mode.
