...
| `raft.wal.min_available_size` | `512MiB` | Reject writes and mark `/__ursula/ready` unavailable below this much free WAL-filesystem space. `0` disables the guard |
| `raft.wal.resume_available_size` | `1GiB` | Free space required before disk-pressure admission clears. Must exceed the minimum |
| `raft.wal.allow_volatile_multi_peer` | `false` | Explicit development/benchmark/chaos opt-in required for more than one peer with a memory WAL |
...
Pick one WAL backend per cluster and use it on every peer. Production clusters
should use `disk`; a multi-peer `memory` configuration fails validation unless
the volatile-mode opt-in is set explicitly.
