...
`--preset` supplies a base configuration before the config file is merged; config-file values win. Pick by node size:
...
| Config key | Default | Purpose |
| ---------- | ------- | ------- |
| `raft.node_id` | — | This node's stable ID; usually overridden per host with `--node-id` |
...
| `raft.wal.path` | none | Required for `backend = "disk"`; Raft logs live under `PATH/raft-log`. Removing the directory wipes state cleanly |
| `raft.peers` | empty | Static gRPC peer list — one `[[raft.peers]]` entry per voter, including this node |
...
When `access_key_id` and `secret_access_key` are omitted, Ursula uses the standard AWS SDK credential chain (instance profile, environment, profile, …) — prefer that over static keys. Set `endpoint` for S3-compatible stores such as MinIO, R2, or TOS.
