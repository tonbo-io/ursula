...
- **Per-group backpressure.** When a group's hot ring exceeds `URSULA_COLD_MAX_HOT_BYTES_PER_GROUP`, appends to that group return `503` with `Retry-After` until cold flush catches up. Per-group, not global or per-client.
...
- **Inter-node encryption.** Peer gRPC (Raft heartbeats, append-entries, snapshots, forwarded writes) runs over h2c. Peers must share a private network.
...
- **Bind to the private interface.** `--listen 10.0.0.X:4437` or a security group / firewall so the listener is unreachable from public networks.
...
- **Use IAM roles for S3.** Omit the static `URSULA_COLD_S3_ACCESS_KEY_ID` / `_SECRET_ACCESS_KEY` and let the AWS SDK credential chain discover credentials.
- **Encrypt data volumes.** Apply full-disk encryption to `--wal-dir` / `--raft-log-dir`.
