...
- **Health/readiness endpoints.** `GET /__ursula/ready` is unauthenticated on
  the client plane and reports only readiness plus WAL free-space guard state.
  `/__ursula/metrics` contains substantially more operational detail; keep the
  node listener private.
