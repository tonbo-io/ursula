...
`--disk-path` selects disk storage when `--storage-backend` is omitted. Do not pass it with `--storage-backend memory`.

### `static gRPC Raft does not support legacy --wal-dir`

The hidden legacy `--wal-dir` flag selects the standalone WAL runtime. Cluster mode needs `--storage-backend disk --disk-path DIR` or `--storage-backend memory`.

Legacy storage flags are compatibility-only and will be removed in a future 0.1.x cleanup.
...
- Disk pressure if `--storage-backend disk` (fsync latency).
...
- `URSULA_COLD_BACKEND` not set, or set to `memory` when you expected `s3`.
