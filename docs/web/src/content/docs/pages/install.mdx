...
Ursula installs two ways: pull the pre-built Docker image from GHCR, or build from source. Standalone release binaries are on the way; the source build is plain Rust plus the standard C toolchain that Cargo expects, with no embedded KV dependency.
...
Every release publishes a multi-arch image containing `ursula`, `ursulactl`, and the `ursulagw` gateway:

```bash
docker pull ghcr.io/tonbo-io/ursula:0.2.0
docker run --rm -p 4437:4437 ghcr.io/tonbo-io/ursula:0.2.0
```

The image runs as a non-root user and exposes port 4437. `latest` tracks the main branch; pin a version tag for anything you keep running. For Kubernetes, the [Helm chart](/docs/deploy-kubernetes) wires this image up for you.
...
- [Deploy on Kubernetes](/docs/deploy-kubernetes): one `helm install` from the published chart
