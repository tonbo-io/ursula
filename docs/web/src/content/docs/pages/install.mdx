...
The image runs as a non-root user and contains both `ursula` (the server) and `ursulactl` (the operator CLI). Images are published only by the reviewed stable-release workflow, so pin the version you want to run.
