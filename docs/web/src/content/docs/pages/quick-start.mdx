...
> Prefer containers? `docker run --rm -p 4437:4437 ghcr.io/tonbo-io/ursula:0.3.1` starts the same single node. See [Install](/docs/install). This page uses a source build.

Start a local node from the workspace root. The default preset is in-memory (no on-disk state, good for kicking the tires):

```bash
cargo run --bin ursula -- --preset default
```

It binds `127.0.0.1:4437`, picks a core count from your CPU, and uses the in-memory engine. For local disk-backed persistence instead, create a small config file:

```bash
cat > ./ursula.toml <<'TOML'
[server]
...
- [Run locally](/docs/run-locally): config files, presets, and durability modes
