...
- **Disk-backed Raft engine** (`raft.wal.backend = "disk"` with
  `raft.wal.path = "DIR"`). OpenRaft groups on the same core share one
  checksummed, versioned journal under `DIR/raft-log`; cross-group fsync
  batching avoids one writer and cache per group. Snapshot plus committed-tail
  replay rebuilds the in-memory state machine after restart, and purge-driven
  online checkpoints reclaim obsolete physical records.
